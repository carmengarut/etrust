    ...
    .then((stream) => {
      ...
      usePhoto.addEventListener('click', async () => {
        try {
          ...
          dispatch(userEdit(user.id, { selfie: user.id + `-selfie.${type}` }))
  ...
  return (
    <div className='kif-container'>
      ...
      <div className='kif-text-container'>
        ...
        <div>
          {t('kyc.make_sure_your_entire_face_is_visible')}
        </div>
        <video src='' id='video' className='kif-video-reverse' playsInline />
